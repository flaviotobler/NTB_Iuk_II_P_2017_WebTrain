...
*/
app.post('/frei', function (req, res)
		{
			console.log("Button Frei pressed"); //Output a debug message
			pfio.digital_write(0, 1); //Turn the switch to the left
			setTimeout(freiaus, 1000); //Turn off the signal after a short period of time
...
*/
app.post('/sperrung', function (req, res) 
	 	{
 			console.log("Button Sperrung pressed"); //Output a debug message
			pfio.digital_write(1, 1); //Turn the switch to the right
			setTimeout(sperrungaus, 1000); //Turn off the signal after a short period of time
...
/*
When the server gets started via nodejs the HTML site signal.html gets called.
*/
